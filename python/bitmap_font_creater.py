...
def open_file():
    ...
    digit_file = tkinter.filedialog.askopenfile(mode='r', **file_options)
    if digit_file:
        digit_filename = digit_file.name
...
def save_digits(digit_file):
    for d in range(10):
        for i in range(16):
            print('0x%02x,' % digits[d * 16 + i], end=' ', file=digit_file)
        print('//', d, file=digit_file)
        
def save_file():
    if digit_filename is None:
        digit_file = tkinter.filedialog.asksaveasfile(mode='w', **file_options)
    else:
